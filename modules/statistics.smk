...
def generate_test_datasets(wildcards):
    try:
        ...
        datasets = safe_expand(template, node_count = [5] + list(range(10, 20)), edge_count = list(range(20, 40)), ensure_strongly_connected = False, seed = 0)
        return safe_expand(DATASET_STATISTICS_JSON, dataset = list(datasets))
